...
using NnhLabLession08.Models;
using System;
using System.Collections.Generic;
using System.Linq;

namespace NnhLab08_Lession08.Controllers
{
	public class NnhProductController : Controller
	{
		...
		// Mock data sản phẩm
		private static List<NnhProduct> _products = new List<NnhProduct>()
		{
			new NnhProduct
			{
				Id = 1,
		...
		// GET: Xem chi tiết
		[HttpGet]
		public IActionResult NnhDetails(int id)
		{
		...
		{
			LoadCategories();
			return View(new NnhProduct());
		}

		// POST: Thêm mới
		[HttpPost]
		[ValidateAntiForgeryToken]
		public IActionResult NnhCreate(NnhProduct product)
		{
		...
		// GET: Chỉnh sửa
		[HttpGet]
		public IActionResult NnhEdit(int id)
		{
			var product = _products.FirstOrDefault(x => x.Id == id);
			LoadCategories();
			return View(product);
		...
		[HttpPost]
		[ValidateAntiForgeryToken]
		public IActionResult NnhEdit(int id, NnhProduct product)
		{
		...
		// GET: Xác nhận xóa
		[HttpGet]
		public IActionResult NnhDelete(int id)
		{
			var product = _products.FirstOrDefault(x => x.Id == id);
			ViewBag.CategoryName = _categories.FirstOrDefault(c => c.Id == product?.CategoryId)?.Name;
			return View(product);
		...
		[ValidateAntiForgeryToken]
		public IActionResult NnhDeleteConfirmed(int id)
		{
			var product = _products.FirstOrDefault(x => x.Id == id);
			if (product != null)
			{
